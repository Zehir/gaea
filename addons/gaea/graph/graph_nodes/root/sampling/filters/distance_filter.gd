...
func _passes_filter(_input_sample: GaeaValue.GridType, cell: Vector3i, graph: GaeaGraph, pouch: GaeaGenerationPouch) -> bool:
	var point: Vector3 = _get_arg(&"to_point", graph, pouch)
	var distance_range: Dictionary = _get_arg(&"distance_range", graph, pouch)
