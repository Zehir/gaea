...
var tween: Tween
var type: GaeaNodeSlot.SlotTypes = GaeaNodeSlot.SlotTypes.NUMBER:
	set(new_value):
		if type != new_value:
			type = new_value
			if not is_part_of_edited_scene():
				resource.title = "Reroute (%s)" % GaeaNodeSlot.SlotTypes.find_key(new_value).capitalize()
			_update_slots()
...
func _update_slots():
	...
	if not is_part_of_edited_scene():
		...
		resource.title = "Reroute (%s)" % GaeaNodeSlot.SlotTypes.find_key(type).capitalize()
#endregion
